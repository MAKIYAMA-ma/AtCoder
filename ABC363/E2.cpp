...
int main() {
    ...
    while(!pq.empty()) {
        ...
        if(d <= y) ans[d]++;
        if(ih > 0 && drp[ih-1][iw] == MAXLL) {
            drp[ih-1][iw] = max(drp[ih][iw], a[ih-1][iw]);
            pq.push(make_pair(drp[ih-1][iw], make_pair(ih-1, iw)));
        }
        if(ih < h-1 && drp[ih+1][iw] == MAXLL) {
            drp[ih+1][iw] = max(drp[ih][iw], a[ih+1][iw]);
            pq.push(make_pair(drp[ih+1][iw], make_pair(ih+1, iw)));
        }
        if(iw > 0 && drp[ih][iw-1] ==  MAXLL) {
            drp[ih][iw-1] = max(drp[ih][iw], a[ih][iw-1]);
            pq.push(make_pair(drp[ih][iw-1], make_pair(ih, iw-1)));
        }
        if(iw < w-1 && drp[ih][iw+1] == MAXLL) {
            drp[ih][iw+1] = max(drp[ih][iw], a[ih][iw+1]);
            pq.push(make_pair(drp[ih][iw+1], make_pair(ih, iw+1)));
        }
    }
