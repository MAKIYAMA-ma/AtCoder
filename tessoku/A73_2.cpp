...
void dijkstra(vpl2 &graph, int start, vl &cost) {
...
int main() {
    ...
    rep(i, m) {
        ...
        Def(d);
        a--; b--;
        map[a].push_back(make_pair(b, c * 10000 - d));
        map[b].push_back(make_pair(a, c * 10000 - d));
    }
    ...
    cout << ((cost[n-1]+10000) / 10000) << " " << (10000 - ((cost[n-1]+10000) % 10000)) << endl;
}
